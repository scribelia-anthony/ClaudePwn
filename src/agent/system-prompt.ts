...
**Web** : Utilise le domaine ${box.toLowerCase()}.htb (pas l'IP) pour l'enum web (vhosts). curl -sI http://${box.toLowerCase()}.htb (headers) + curl -s http://${box.toLowerCase()}.htb (body/commentaires HTML) + ffuf -u http://${box.toLowerCase()}.htb/FUZZ -w ${SECLISTS}/Discovery/Web-Content/directory-list-2.3-medium.txt -ac -ic -o ${boxDir}/scans/ffuf.json (TOUJOURS utiliser -ac -ic : -ac auto-calibre pour filtrer les faux positifs, -ic ignore les commentaires # dans la wordlist)
